import {Bag,COLORS,COLS,ROWS,HIDDEN_ROWS,createBoard,makePiece,collides,mergePiece,clearLines,rotateMatrix,scoreFor,attackFor,isPerfectClear,isTSpin,ghostY} from './game-core.js';
...
const SETTINGS_KEY='block-duel-wu03-settings';
const defaults={mode:'buttons',sensitivity:'normal',haptic:true,shake:true};
let settings=loadSettings();
const freshState=()=>({board:createBoard(),bag:new Bag(),queue:[],piece:null,hold:null,holdUsed:false,score:0,lines:0,combo:-1,b2b:0,level:1,attack:0,paused:false,over:false,mode:settings.mode,elapsed:0,last:performance.now(),dropAcc:0,dropMs:700,lastActionWasRotate:false});
...
function spawn(){fillQueue();state.piece=makePiece(state.queue.shift());state.holdUsed=false;state.lastActionWasRotate=false;fillQueue();if(collides(state.board,state.piece)){state.over=true;showOverlay('게임 오버');}drawAll();}
function reset(){Object.assign(state,freshState());hideOverlay();spawn();updateHud();}
function move(dx,dy){if(state.paused||state.over)return false;if(!collides(state.board,state.piece,dx,dy)){state.piece.x+=dx;state.piece.y+=dy;state.lastActionWasRotate=false;drawAll();return true;}return false;}
function rotate(){if(state.paused||state.over)return;const m=rotateMatrix(state.piece.matrix);for(const k of [0,-1,1,-2,2]){if(!collides(state.board,state.piece,k,0,m)){state.piece.matrix=m;state.piece.x+=k;state.piece.rotation=(state.piece.rotation+1)%4;state.lastActionWasRotate=true;haptic('light');drawAll();return;}}}
function soft(){if(move(0,1)){state.score+=1;updateHud();}else lock();}
function hard(){if(state.paused||state.over)return;const y=ghostY(state.board,state.piece);state.score+=(y-state.piece.y)*2;state.piece.y=y;state.lastActionWasRotate=false;lock(true);}
function hold(){if(state.paused||state.over||state.holdUsed)return;const current=state.piece.type;if(state.hold){state.piece=makePiece(state.hold);state.hold=current;}else{state.hold=current;fillQueue();state.piece=makePiece(state.queue.shift());fillQueue();}state.holdUsed=true;state.lastActionWasRotate=false;haptic('medium');drawAll();}
...
function effect(level){if(!settings.shake)return;wrap.classList.remove('shake-small','shake-strong');void wrap.offsetWidth;wrap.classList.add(level==='strong'?'shake-strong':'shake-small');setTimeout(()=>wrap.classList.remove('shake-small','shake-strong'),220);}
...
function drawBoard(){const cell=boardCanvas.width/COLS;ctx.clearRect(0,0,boardCanvas.width,boardCanvas.height);ctx.fillStyle='#05070b';ctx.fillRect(0,0,boardCanvas.width,boardCanvas.height);ctx.strokeStyle='#121727';ctx.lineWidth=1;for(let x=0;x<=COLS;x++){ctx.beginPath();ctx.moveTo(x*cell,0);ctx.lineTo(x*cell,boardCanvas.height);ctx.stroke();}for(let y=0;y<=ROWS;y++){ctx.beginPath();ctx.moveTo(0,y*cell);ctx.lineTo(boardCanvas.width,y*cell);ctx.stroke();}for(let y=HIDDEN_ROWS;y<state.board.length;y++)for(let x=0;x<COLS;x++)if(state.board[y][x])drawCell(state.board[y][x],x,y-HIDDEN_ROWS,cell);if(!state.piece)return;const gy=ghostY(state.board,state.piece);state.piece.matrix.forEach((r,y)=>r.forEach((v,x)=>{if(v&&gy+y>=HIDDEN_ROWS)drawCell(state.piece.type,state.piece.x+x,gy+y-HIDDEN_ROWS,cell,.18);}));state.piece.matrix.forEach((r,y)=>r.forEach((v,x)=>{if(v&&state.piece.y+y>=HIDDEN_ROWS)drawCell(state.piece.type,state.piece.x+x,state.piece.y+y-HIDDEN_ROWS,cell);}));}
...
document.querySelectorAll('[data-action]').forEach(btn=>{const a=btn.dataset.action;btn.addEventListener('pointerdown',e=>{e.preventDefault();btn.setPointerCapture?.(e.pointerId);btn.classList.add('is-pressed');action(a);haptic('light');const press={button:btn,delayTimer:null,repeatTimer:null};activePresses.set(e.pointerId,press);if(['left','right','soft'].includes(a)){const delay=a==='soft'?100:145;const interval=a==='soft'?42:58;press.delayTimer=setTimeout(()=>{press.repeatTimer=setInterval(()=>action(a),interval);},delay);}});['pointerup','pointercancel','lostpointercapture'].forEach(type=>btn.addEventListener(type,e=>stopPress(e.pointerId)));});
...
let gesture=null;
wrap.addEventListener('pointerdown',e=>{if(state.mode!=='swipe'||state.paused||state.over)return;e.preventDefault();gesture={pointerId:e.pointerId,startX:e.clientX,startY:e.clientY,lastX:e.clientX,lastY:e.clientY,t:performance.now(),moved:false,axis:null,horizontalSteps:0,softSteps:0};wrap.setPointerCapture?.(e.pointerId);});
wrap.addEventListener('pointermove',e=>{if(!gesture||gesture.pointerId!==e.pointerId||state.mode!=='swipe')return;e.preventDefault();const p=getSwipePreset(settings.sensitivity);const totalDx=e.clientX-gesture.startX,totalDy=e.clientY-gesture.startY;gesture.axis=resolveAxis(totalDx,totalDy,gesture.axis,p.axisLockPx);if(gesture.axis)gesture.moved=true;if(gesture.axis==='x'){const desired=stepsFromDistance(totalDx,p.stepPx);while(gesture.horizontalSteps<desired){move(1,0);gesture.horizontalSteps++;}while(gesture.horizontalSteps>desired){move(-1,0);gesture.horizontalSteps--;}}else if(gesture.axis==='y'&&totalDy>0){const desired=Math.max(0,stepsFromDistance(totalDy,p.stepPx));while(gesture.softSteps<desired){soft();gesture.softSteps++;}}gesture.lastX=e.clientX;gesture.lastY=e.clientY;});
function endGesture(e){if(!gesture||gesture.pointerId!==e.pointerId||state.mode!=='swipe')return;const dx=e.clientX-gesture.startX,dy=e.clientY-gesture.startY,dt=performance.now()-gesture.t;const result=classifyGesture({dx,dy,dt,moved:gesture.moved,softSteps:gesture.softSteps},settings.sensitivity);if(result==='rotate')rotate();else if(result==='hold')hold();else if(result==='hard')hard();gesture=null;}
wrap.addEventListener('pointerup',endGesture);wrap.addEventListener('pointercancel',()=>{gesture=null;});
function openSettings(){$('#mode-select').value=state.mode;$('#sensitivity-select').value=settings.sensitivity;$('#haptic-toggle').checked=settings.haptic;$('#shake-toggle').checked=settings.shake;$('#settings-sheet').classList.remove('hidden');}
function closeSettings(){$('#settings-sheet').classList.add('hidden');}
$('#settings-btn').addEventListener('click',openSettings);$('#settings-close').addEventListener('click',closeSettings);document.querySelector('[data-close-settings]').addEventListener('click',closeSettings);$('#mode-select').addEventListener('change',e=>applyMode(e.target.value));$('#sensitivity-select').addEventListener('change',e=>{settings.sensitivity=e.target.value;saveSettings();});$('#haptic-toggle').addEventListener('change',e=>{settings.haptic=e.target.checked;saveSettings();haptic('medium');});$('#shake-toggle').addEventListener('change',e=>{settings.shake=e.target.checked;saveSettings();});
...
const resizeObserver=new ResizeObserver(resizeBoard);resizeObserver.observe($('#game-stage'));const refreshLayout=()=>{updateViewportHeight();requestAnimationFrame(resizeBoard);};window.visualViewport?.addEventListener('resize',refreshLayout);window.visualViewport?.addEventListener('scroll',refreshLayout);window.addEventListener('orientationchange',()=>setTimeout(refreshLayout,160));window.addEventListener('resize',refreshLayout);
applyMode(settings.mode,{persist:false});reset();refreshLayout();requestAnimationFrame(loop);
